...
import (
	...
	"github.com/hashicorp/waypoint-plugin-examples/filepath/utils"
...
func (r *Registry) push(
	...
	path, err := utils.CopyFile(img.Path, r.config.Directory)
	if err != nil {
		st.Step(terminal.StatusError, "Unable to copy file to registry")

		return &Artifact{}, status.Errorf(
			codes.Internal,
			"Unable to copy file %s to %s: %s", img.Path, r.config.Directory, err.Error(),
		)
	}
